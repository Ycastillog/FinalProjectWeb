...
class Incidents extends BaseModel
{
    ...
    public function create(array $d, ?int $userId): int {
        $st = $this->db->prepare(
            "INSERT INTO incidents
             (title, description, occurred_at, follow_up_at,
              province_id, municipality_id, barrio_id,
              lat, lng, dead, injured, loss_rd, social_link, photo,
              status, user_id)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        );
        $st->execute([
            ...
            $d['dead']?:null, $d['injured']?:null, $d['loss_rd']?:null,
            $d['social_link']?:null, $d['photo']??null,
            'pending', $userId
        ]);
    ...
    public function agendaUpcoming(int $days = 30): array {
        $st = $this->db->prepare(
          "SELECT i.id, i.title, i.follow_up_at, i.status, i.occurred_at
           FROM incidents i
           WHERE i.follow_up_at IS NOT NULL
             AND i.follow_up_status='pending'
             AND i.follow_up_at BETWEEN NOW() AND (NOW() + INTERVAL ? DAY)
    ...
    public function setDone(int $id): void {
        $this->db->prepare("UPDATE incidents SET follow_up_status='done' WHERE id=?")->execute([$id]);
    }
}
